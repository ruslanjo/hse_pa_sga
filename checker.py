...
def check():
    ...
    os.system(f'python3 {solution_path} build --dataset {wikipedia_path} --index {index_path}')
    ...
    for test_number, in_filename in enumerate(test_input_data_files, start=1):
        ...
        try:
            command = f'python3 {solution_path} query --index {index_path} --query_file {in_filename} > {out_filename}'
            os.system(command)
